...
namespace ge {
// Relaunch this same executable as a fresh, detached process. Used by the
// ONLINE pause-menu tab's "Save & Restart": the new instance reads the
// just-written ge.toml (new username / server / online-enable) at boot, then
// the caller tears the current process down. Launching a second instance of a
// running exe is fine on Windows -- the image file is opened share-read.
void LaunchSelfDetached() {
  ...
  std::wstring workdir =
      (slash == std::wstring::npos) ? std::wstring() : full.substr(0, slash);
...
std::atomic<uint32_t> g_dbgnow_calls{
    0}; // increments each ge_dbg_now (guest polling sub_82198C28)

void ge_watchdog_thread() {
  uint8_t *base = rex::system::kernel_state()->memory()->virtual_membase();
  uint32_t last_wpi = 0xFFFFFFFFu, last_rpi = 0, last_present = 0,
           last_submit = 0;
  uint32_t present_at_stall_start = 0, dbg_at_stall_start = 0,
  ...
  for (;;) {
    std::this_thread::sleep_for(
        std::chrono::milliseconds(500)); // 500ms poll (was 250ms)
    auto *cp = ge_cp();
    if (!cp)
      continue;
    ...
    if (present_alive && !ring_moved) {
      ...
      // WAIT_REG_MEM polling idblk for ==0 (the semaphore the render writes 0
      // to release the CP). The render is parked waiting on GPU completion ->
      ...
      if (stall >= 2 && dev && idblk && idblk < 0xFFFFFFFEu) {
        ...
        if (!recover_fired) {
          recover_fired = true;
          REXKRNL_INFO(
              "GEWATCHDOG RECOVERY: released CP semaphore (idblk={:#x} := 0)",
              idblk);
        }
      }
      if (stall >= 6 && !logged) { // ~1.5s of present-but-no-ring
        ...
        REXKRNL_INFO("GEWATCHDOG STALL: ring rpi={:#x} wpi={:#x} [{}] | "
                     "present#={} (+{}/stall) | "
                     "dbgnow_polls={} (+{}/stall) | submit={} completed={} "
                     "target={} presented={} skipbit={} "
                     "| dev={:#x} idblk={:#x}",
                     rpi, wpi, (rpi == wpi ? "DRAINED" : "PENDING"), present,
                     present - present_at_stall_start, dbg,
                     dbg - dbg_at_stall_start, submit, completed, target,
                     presented, skip, dev, idblk);
        REXKRNL_INFO(
            "GEWATCHDOG -> completion={} | presenting={} | producer={} | "
            "polling={}",
            (submit > completed ? "GPU BEHIND (completion not delivered)"
                                : "caught up"),
            (submit > presented ? "frames NOT presenting" : "caught up"),
            (submit != submit_at_stall_start ? "ALIVE (submitting)"
                                             : "STALLED (not submitting)"),
            (dbg != dbg_at_stall_start ? "guest spinning in sub_82198C28"
                                       : "guest NOT polling"));
        // Render gate: frame loop runs render+present only when
        // dword_8242043C&2 (sub_8209E1C0). Set by sub_8209E1D0(mode): mode 3 at
        // init (enabled), mode 1 = bit clear = render skipped every frame =
        // freeze.
        uint32_t rg = LD32(base, 0x8242043Cu);
        REXKRNL_INFO(
            "GEWATCHDOG -> render-gate dword_8242043C={} -> render+present {}",
            rg,
            (rg & 2u) ? "ENABLED"
                      : "DISABLED (frame loop skips render = FREEZE)");
        // Device flags gating the present/submit (a1 = dev). +21516 != 0 => the
        // present SKIPS VdSwap (no screen update) and sub_821A4D50 takes its
        // alt path; +22280&4 gates the GPU-completion wait; +10941/+10943 =
        // skip bits.
        if (dev) {
          ...
          REXKRNL_INFO("GEWATCHDOG -> devflags +21516(VdSwap-skip if "
                       "!=0)={:#x} | +22280&4(gpu-wait)={} | "
                       "+22276={:#x} | +21604={} +21600={} (ring) | "
                       "+10941={:#x} +10943={:#x}",
                       f21516, (f22280 & 4u), f22276, f21604, f21600, b10941,
                       b10943);
          ...
          REXKRNL_INFO(
              "GEWATCHDOG -> vblank ctx[4133]={} | GPU fences read={} write={} "
              "[{}]",
              vbl, fr, fw,
              (fr != fw ? "PENDING -- fences NOT retiring" : "drained"));
        }
        // Frame counter dword_8308851C is updated each frame AFTER the frame-
        // limiter (0x82189e64). Sample it twice: if FROZEN, the main thread
        // never exits the frame-limiter (clock/timebase not advancing for it);
        // if it ADVANCES, the main thread cycles and the render is skipped
        // after.
        ...
        REXKRNL_INFO("GEWATCHDOG -> frameCounter 0x8308851C {}->{} [{}] | "
                     "guestTimebase {}->{} [{}]",
                     fc1, fc2,
                     (fc1 != fc2
                          ? "ADVANCING (main thread cycles; render skipped "
                            "after limiter)"
        ...
        if (ks2) {
          auto threads =
              ks2->object_table()->GetObjectsByType<rex::system::XThread>();
          for (auto &th : threads) {
            if (!th)
              continue;
              ...
              if (sp >= 0x10000u && sp < 0xC0000000u) {
                ...
                for (uint8_t *pp = hsp; pp + 4 <= send && soff < 460; pp += 4) {
                  ...
                  if (val >= 0x82000000u && val < 0x84000000u) {
                    int n = std::snprintf(sbuf + soff, sizeof(sbuf) - soff,
                                          "%x ", val);
                    if (n > 0)
                      soff += n;
                  }
                }
                REXKRNL_INFO("GEWATCHDOG   STACK start={:#x} sp={:#x}: {}", sa,
                             sp, sbuf);
              }
            }
            if (rw) {
              // a1 (worker struct) = r28; event = a1[2] = a1+0x20 (= r29);
              // queue = a1[3]: Flink/submit = a1+0x38, Blink/processed =
              // a1+0x3C. wait is INFINITE when a1->SignalState(a1+4) !=
              // *(v3+368), v3 = *a1.
              uint32_t bw = c->r28.u32;
              auto sLD = [&](uint32_t ga) -> uint32_t {
                return (ga >= 0x1000u && ga < 0x50000000u) ? LD32(base, ga)
                                                           : 0xDEADBEEFu;
              };
              uint32_t v3 = sLD(bw);
              ...
              REXKRNL_INFO("GEWATCHDOG   WORKER a1={:#x} queue Flink/submit={} "
                           "Blink/proc={} [{}] | "
                           "SignalState={} v3={:#x} *(v3+368)={} -> wait={}",
                           bw, subq, procq,
                           (subq == procq ? "EMPTY (producer stopped feeding)"
                                          : "PENDING (LOST WAKEUP!)"),
                           sig, v3, v3f,
                           (sig != v3f ? "INFINITE" : "30ms-timeout"));
            }
          }
          // Rapid-sample the main game thread (start 0x8235e4a8): it spends
          // most time in the frame-limiter, so one snapshot misses the render
          // path. Sample lr many times (yielding so it keeps running) -> the
          // set of unique guest PCs = its per-frame code path, revealing which
          // render subsystem call it reaches/skips.
          for (auto &th : threads) {
            if (!th)
              continue;
            ...
            // spinning -> doesn't starve it, it keeps cycling). Log only
            // snapshots where it is OUTSIDE the frame-limiter -> in the
            // per-frame render path -> the render call chain + the skipped
            // 3D-submit branch.
            {
              int logged = 0;
              for (int snap = 0; snap < 160 && logged < 12; snap++) {
                ...
                if (!in_lim && pc >= 0x82000000u && pc < 0x84000000u) {
                  ...
                  if (sp >= 0x10000u && sp < 0xC0000000u) {
                    ...
                    for (uint8_t *pp = hsp; pp + 4 <= send && fo < 580;
                         pp += 4) {
                      uint32_t v;
                      std::memcpy(&v, pp, 4);
                      v = __builtin_bswap32(v);
                      if (v >= 0x82000000u && v < 0x84000000u) {
                        int n =
                            std::snprintf(fb + fo, sizeof(fb) - fo, "%x ", v);
                        if (n > 0)
                          fo += n;
                      }
                    }
...
void ge_dbg_now(PPCRegister &r9, PPCRegister &r30) {
  ...
  // Cache the CP pointer — traversing the object graph every poll iteration
  // (kernel_state -> emulator -> IGraphics -> CP) is wasteful.
  static rex::graphics::CommandProcessor *s_cp = nullptr;
  uint32_t dev = ctx->r29.u32;
  uint32_t idblk = ctx->r11.u32;
  uint32_t ws = ctx->r31.u32;
  uint32_t cpc = 0, rpi = 0, wpi = 0;

  // Refresh the cached CP pointer lazily — only traverses the graph when null
  // or after a device reset (detected by dev changing).
  if (!s_cp || s_cp != ge_cp()) {
    s_cp = ge_cp();
  }
  if (s_cp) {
    cpc = s_cp->counter();
    rpi = static_cast<CPProbe *>(s_cp)->rpi();
    wpi = static_cast<CPProbe *>(s_cp)->wpi();
  }

  // Feed the freeze watchdog (relaxed atomics, checkpoint only every ~1s).
  static thread_local uint32_t s_last_dbg = 0;
  if ((t - s_last_dbg) > 50000000u) { // ~1s at 50MHz
    s_last_dbg = t;
  ...
  // GPU completion: drawn = CP counter advanced past present-sampled value,
  // OR ring buffer drained. Avoid redundant work in the hot path.
  bool drawn = (cpc != g_present_cpcnt.load(std::memory_order_relaxed)) ||
               (wpi != 0u && rpi == wpi);

  // Completion stall guard. When the CP hasn't finished this frame and the
  // guest is spinning: yield the CPU so the CP worker thread (running on its
  // own core) can advance. Only yield once every 16 polls (~16µs) to balance
  // wake-up latency vs CPU overhead.
  static thread_local uint32_t s_wait_start = 0;
  static thread_local bool s_waiting = false;
  static thread_local uint32_t s_yield_skip = 0;
  if (!drawn) {
    if (!s_waiting) {
      s_waiting = true;
      s_wait_start = t;
    ...
    // Yield every 16th poll (NOT every poll) so the guest still gives the
    // CP worker CPU without overflowing the scheduler with yield syscalls.
    if (!drawn && ((++s_yield_skip & 0xFu) == 0)) {
      std::this_thread::yield();
    }
  } else {
    s_waiting = false;
...
// 0x820F7750 (cmpwi r30,8 / blt loc_820F768C) IN THE PARENT sub_820F73F8, so
// the whole loop -- including the loop-back to 0x820F768C -- stays in one
// function and resolves. (Routing through a separate ge_cont_820F7750 left that
// loop-back branch cross-function -> REX_FATAL when the loop ran, e.g. at the
// main menu.)
...
bool ge_hook_830E0200(PPCRegister &r31, PPCRegister &r29, PPCRegister &r28,
  ...
  // (0x820C4854 b loc_820C485C) -> only the loop-back reaches it, so it is not
  // a standalone block. Do its r11 reload here and jump to 0x820C485C (which IS
